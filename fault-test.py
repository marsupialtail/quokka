...
r.flushall()
input_actors_a = {k: InputS3CSVNode.options(max_concurrency = 2, num_cpus = 0.001).remote(0,k,"yugan","a-big.csv",["key"] + ["avalue" + str(i) for i in range(100)], NUM_MAPPERS, ("a","b")) for k in range(NUM_MAPPERS)}
input_actors_b = {k: InputS3CSVNode.options(max_concurrency = 2, num_cpus = 0.001).remote(1,k,"yugan","b-big.csv",["key"] + ["bvalue" + str(i) for i in range(100)], NUM_MAPPERS, ("a","b")) for k in range(NUM_MAPPERS)}
parents = {0:input_actors_a, 1: input_actors_b}
join_executor = PolarJoinExecutor(on="key")
join_actors = {i: NonBlockingTaskNode.options(max_concurrency = 2, num_cpus = 0.001).remote(2,i, {0:0,1:1}, None, join_executor, parents, ("bump","bump")) for i in range(NUM_JOINS)}
...
def input_actor_failure_test():
    ...
    # immediately restart this actor
    for actor_id in to_dies:
        input_actors_a[actor_id] =  InputS3CSVNode.options(max_concurrency = 2, num_cpus = 0.001).remote(0,actor_id,"yugan","a-big.csv",["key"] + ["avalue" + str(i) for i in range(100)], NUM_MAPPERS, ckpt = "ckpt-0-"+str(actor_id)+".pkl") 
