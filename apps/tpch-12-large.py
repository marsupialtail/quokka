...
from dataset import InputMultiCSVDataset, InputCSVDataset, InputMultiParquetDataset
import pandas as pd
import ray
import os
import polars
...
ips = ['localhost', '172.31.5.171', '172.31.4.37', '172.31.14.231', '172.31.11.101', '172.31.9.101', '172.31.2.245', '172.31.11.192', '172.31.4.239', '172.31.9.51', '172.31.9.174', '172.31.9.175', '172.31.14.44', '172.31.3.237', '172.31.9.30', '172.31.10.223', '172.31.2.203', '172.31.5.139', '172.31.5.69', '172.31.11.135', '172.31.14.5', '172.31.12.5', '172.31.4.66', '172.31.10.194', '172.31.1.25', '172.31.12.220', '172.31.0.23', '172.31.5.216', '172.31.11.211', '172.31.7.85', '172.31.0.15', '172.31.0.207']
workers = 32
...
if sys.argv[2] == "csv":
    if sys.argv[1] == "small":
        ...
        lineitem_csv_reader = InputMultiCSVDataset("tpc-h-csv", "lineitem/lineitem.tbl", lineitem_scheme , sep="|", stride = 128 * 1024 * 1024)
        orders_csv_reader = InputMultiCSVDataset("tpc-h-csv", "orders/orders.tbl", order_scheme , sep="|", stride = 128 * 1024 * 1024)
        lineitem_csv_reader.get_own_state(8 * workers)
        orders_csv_reader.get_own_state(4 * workers)

        lineitem = task_graph.new_input_reader_node(lineitem_csv_reader, {ip:8 for ip in ips[:workers]}, batch_func = lineitem_filter)
        orders = task_graph.new_input_reader_node(orders_csv_reader, {ip:4 for ip in ips[:workers]}, batch_func = orders_filter)
