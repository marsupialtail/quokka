...
from sql import StatelessExecutor
import pandas as pd
import numpy as np
import ray
import os
...
class MatMulExecutor(StatelessExecutor):
    ...
    def execute(self,batches, stream_id, executor_id):
        ...
        batch = pd.concat(batches).numpy()[:,1:]
        if stream_id == 0:
        ...
        if stream_id == 1:
            ...
            col_start = sum([i.shape[0] for i in self.state1])
            col_end = col_start + batch.shape[0]
            row_start = 0
...
A = task_graph.new_input_csv("yugan","matrix.csv",["key"] + ["avalue" + str(i) for i in range(1000)],{'localhost':4})
AT = task_graph.new_input_csv("yugan","matrix.csv",["key"] + ["avalue" + str(i) for i in range(1000)],{'localhost':4})
