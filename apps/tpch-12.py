...
if sys.argv[2] == "csv":
    if sys.argv[1] == "small":
        ...
        orders = task_graph.new_input_csv("tpc-h-csv","orders/orders.tbl.1",order_scheme,{'localhost':4, '172.31.11.134':4},batch_func=orders_filter, sep="|")
        lineitem = task_graph.new_input_csv("tpc-h-csv","lineitem/lineitem.tbl.1",lineitem_scheme,{'localhost':8, '172.31.11.134':8},batch_func=lineitem_filter, sep="|")
...
#        orders = task_graph.new_input_multiparquet("tpc-h-parquet","orders.parquet",{'localhost':4},columns = ['o_orderkey','o_orderpriority'], batch_func = orders_filter_parquet)
        ...
        lineitem = task_graph.new_input_multiparquet("tpc-h-parquet","lineitem.parquet", {'localhost':1,'172.31.11.134':1},columns=['l_shipdate','l_commitdate','l_shipmode','l_receiptdate','l_orderkey'], filters= [('l_shipmode', 'in', ['SHIP','MAIL']),('l_receiptdate','<',compute.strptime("1995-01-01",format="%Y-%m-%d",unit="s")), ('l_receiptdate','>=',compute.strptime("1994-01-01",format="%Y-%m-%d",unit="s"))], batch_func=lineitem_filter_parquet)
        orders = task_graph.new_input_multiparquet("tpc-h-parquet","orders.parquet",{'localhost':1,'172.31.11.134':1},columns = ['o_orderkey','o_orderpriority'], batch_func = orders_filter_parquet)
...
output_stream = task_graph.new_non_blocking_node({0:orders,1:lineitem},None,join_executor,{'localhost':1, '172.31.11.134':1}, {0:"o_orderkey", 1:"l_orderkey"})
