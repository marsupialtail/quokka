...
Status BuildBloomFilter( int64_t hardware_flags,
  ...
  BloomFilterBuildStrategy strategy == BloomFilterBuildStrategy::SINGLE_THREADED;
  auto builder = BloomFilterBuilder::Make(strategy);

  RETURN_NOT_OK(builder->Begin(1, hardware_flags, pool, num_rows,
