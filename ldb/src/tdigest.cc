...
class TDigest::TDigestImpl {
 public:
  ...
  double Quantile(double q) const {
    ...
    ASSERT_MSG(ci<  td.size(), "tdigest fail");
    ...
    if (diff > 0) {
      if (ci_right == td.size() - 1) {
        ...
        ASSERT_MSG(weight_sum == total_weight_, "tdigest fail");
        const Centroid* c = &td[ci_right];
        ASSERT_MSG(c->weight > 2, "tdigest fail");
...
void TDigest::add_arrow(uintptr_t arrowArrayPtr, uintptr_t arrowSchemaPtr) {

    BufferView * buffer = bufferFromArrow(arrowArrayPtr, arrowSchemaPtr);
    
    auto start = std::chrono::high_resolution_clock::now();    
    for (int i = 0; i < buffer->length(); i++) {
        // use the [] operator on BufferView
        Add(buffer->valueDouble(i));
    }

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end-start;
    std::cout << "add_arrow took " << diff.count() << " s" << std::endl;
    std::cout << buffer->length() << " values" << std::endl;
    return;
}


NTDigest::NTDigest(uint32_t n, uint32_t delta, uint32_t buffer_size){
  tdigests_.reserve(n);
    for (uint32_t i = 0; i < n; ++i) {
        tdigests_.emplace_back(delta, buffer_size);
    }
}

NTDigest::~NTDigest() = default;
NTDigest::NTDigest(NTDigest&&) = default;
NTDigest& NTDigest::operator=(NTDigest&&) = default;

void NTDigest::add_arrow(uint32_t k, uintptr_t arrowArrayPtr, uintptr_t arrowSchemaPtr) {
    BufferView * buffer = bufferFromArrow(arrowArrayPtr, arrowSchemaPtr);

    for (int i = 0; i < buffer->length(); i++) {
        // use the [] operator on BufferView
        tdigests_[k].Add(buffer->valueDouble(i));
    }
    return;
}

void NTDigest::batch_add_arrow(std::vector<uintptr_t> arrowArrayPtrs, std::vector<uintptr_t> arrowSchemaPtrs) {
    ASSERT_MSG(arrowArrayPtrs.size() == arrowSchemaPtrs.size(), "arrowArrayPtrs and arrowSchemaPtrs must be the same size");
    ASSERT_MSG(arrowArrayPtrs.size() == tdigests_.size(), "arrowArrayPtrs and tdigests_ must be the same size");

    #pragma omp parallel for
    for (uint32_t i = 0; i < arrowArrayPtrs.size(); i++) {
        add_arrow(i, arrowArrayPtrs[i], arrowSchemaPtrs[i]);
    }
}

double NTDigest::Quantile(uint32_t k, double q) const {
  return tdigests_[k].Quantile(q);    
}